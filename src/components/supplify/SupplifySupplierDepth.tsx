...
export function SupplifySupplierDepth() {
  return (
    <section
      data-theme="light"
      className="relative overflow-hidden border-t border-ink/8 bg-supplify-mist py-20 md:py-24"
    >
      <div className="relative mx-auto max-w-[90rem] px-6 md:px-10 lg:px-14">
        <Reveal>
          <SupplifyEyebrow theme="light">Supplier ops depth</SupplifyEyebrow>
          ...
          {SUPPLIER_DEPTH_ITEMS.map((item, i) => {
            ...
            return (
              <Reveal key={item.id} delay={0.04 + i * 0.04}>
                <article className="group flex w-[min(88vw,20rem)] shrink-0 flex-col rounded-2xl border border-ink/8 bg-white p-5 shadow-sm transition-[border-color,box-shadow,transform] duration-300 hover:-translate-y-0.5 hover:border-supplify/25 hover:shadow-editorial sm:w-[18rem]">
                  ...
                  <p className="mt-2 font-sans text-sm leading-[1.65] text-ink-muted">
                    {item.body}
                  </p>
                  <div className="mt-4">
                    <ProductScreenshot
